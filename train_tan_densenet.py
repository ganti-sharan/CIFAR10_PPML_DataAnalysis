...
from densenet import DenseNet, _DenseLayer
            ...
            if is_updated:
                ...
                if is_main_worker:
                    ...
                    if nb_steps % args.freq_log == 0:
                        ...
                        try:
                            with open(f'train_acc_densenet+{epoch}.json' , 'w') as file:
                                json.dump(train_accuracy, file)
                            print('here')
                        except:
                            with open(f'train_acc_densenet+{epoch}.json', 'r') as file:
                    ...
                    if nb_steps % args.freq_log_val == 0:
                        ...
                        try:
                            with open(f'train_ema_densenet+{epoch}.json' , 'w') as file:
                                json.dump(train_acc_ema, file)
                            print('here')
                        except:
                            with open(f'train_ema_densenet+{epoch}.json', 'r') as file:
                                data_list = json.load(file)
